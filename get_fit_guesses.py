...
def main(inputConfig,blinded,tag,nslices=0,sigma=25):
    ...
    rebinnedYPass = copyHistWithNewYbounds(rebinnedXPass,'rebinnedYPass',yBinWidth,ymin,ymax)
    rebinnedYFail = copyHistWithNewYbounds(rebinnedXFail,'rebinnedYFail',yBinWidth,ymin,ymax)
    ...
    if nslices > 0:
        ...
        # new_y_bins_array = array('d',[900.0, 1000.0, 1100.0, 1200.0, 1300.0, 1400.0, 1500.0, 1600.0, 1700.0, 1800.0, 1900.0, 2000.0, 2200.0, 2400.0, 2600.0, 2800.0, 3200.0, 4000.0])    

    else:
        new_y_bins = [yBinWidth*i+ymin for i in range(ynbins+2)]
    ...
    if blinded:
        ...
        if nslices > 0:
            ...
            # rebinnedYPass = copyHistWithNewYbounds(blindedPass,'rebinnedPass',yBinWidth,ymin,ymax)
            # rebinnedYFail = copyHistWithNewYbounds(blindedFail,'rebinnedFail',yBinWidth,ymin,ymax)

            RpfToRemap = rebinnedPass.Clone('rebinnedRpf')
            RpfToRemap.Divide(rebinnedFail)
        else:
            RpfToRemap = blindedPass.Clone('rebinnedRpf')
            RpfToRemap.Divide(blindedFail)
    ...
    # Otherwise just get the Rpf
        ...
        if nslices > 0:
            ...
            RpfToRemap = rebinnedXPass.Clone('rebinnedRpf')
            RpfToRemap.Divide(rebinnedXFail)
    ...
    psuedo2D_Rpf = TF2('psuedo2D_Rpf',funcString,0,1,0,1)
    paramIndex = 0
    for ix in range(nxparams):
